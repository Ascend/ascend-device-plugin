...
if [ ${ASCNED_TYPE} == "310"  ]; then
  ...
  sed -i "s#deviceplugin  --useAscendDocker=\${USE_ASCEND_DOCKER}#deviceplugin --mode=ascend310 --useAscendDocker=${USE_ASCEND_DOCKER}#g" ${TOP_DIR}/ascendplugin.yaml
  LIBDRIVER="/driver/lib64"
fi
sed -i "s/Ascend[0-9]\{3\}/${TYPE}/g" ${TOP_DIR}/deviceplugin.yaml
sed -i "s#ath: /usr/local/Ascend/driver#ath: ${ASCNED_INSTALL_PATH}/driver#g" ${TOP_DIR}/deviceplugin.yaml
